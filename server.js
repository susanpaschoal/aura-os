...
app.post('/api/estoque/retirar', auth, async (req, res) => {
    ...
    const item = await Estoque.findOne({ 
        where: { id, EmpresaId: req.session.user.EmpresaId } 
    });
...
app.get('/login', (req, res) => {
    ...
    <div style="display:flex; justify-content:center; align-items:center; width:100%">
        <div class="stat-card" style="width:350px">
            ...
            <p style="text-align:center; font-size:12px; color:var(--sub); margin-top:15px">
                Novo? Cadastre sua empresa:
            </p>
            ...
            <form action="/api/cadastro" method="POST">
                ...
                <button class="btn-outline" style="width:100%">CADASTRAR</button>
            </form>
        </div>
    </div>`);
});
...
app.get('/', auth, (req, res) => {
    ...
    <script>
        ...
        function renderDash() {
            ...
            document.getElementById('view').innerHTML = \`
                ...
                <div class="stats-grid">
                    <div class="stat-card"><h4>ATIVOS</h4><h2>\${alug}</h2></div>
                    <div class="stat-card"><h4>ESTOQUE</h4><h2>\${comp}</h2></div>
                    <div class="stat-card"><h4>MANUTENÇÃO</h4><h2>\${manu}</h2></div>
                </div>
            \`;
        }
        ...
        async function add() {
            ...
            const res = await fetch('/api/estoque/add', {
                method:'POST',
